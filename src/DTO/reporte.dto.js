...
export const reporteDTO = async (reporte) => {
    ...
    return {
        ...
        fechas: {
            fecha_inicio: (reporte.rep_inicio ? formatDate_YMD(reporte.rep_inicio) : ''),
            fecha_fin: (reporte.rep_fin ? formatDate_YMD(reporte.rep_fin) : '')
        },
        recompensa: {
            tieneRecompensa: (reporte.rep_recompensa ? 1 : 2),
            cantidad: (reporte.rep_monto_recompensa ? '$' + reporte.rep_monto_recompensa : '')
        },
